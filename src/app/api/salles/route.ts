...
export async function GET() {
  console.log('➡️ GET /api/salles: Received request');
  try {
    if (!prisma.classroom) {
    ...
    console.log(`⬅️ GET /api/salles: Success, found ${classrooms.length} classrooms.`);
    return NextResponse.json(classrooms);
  } catch (error: any) {
    console.error('❌ GET /api/salles: Error fetching classrooms:', error);
    if (error.stack) {
        console.error("Stack trace:", error.stack);
    }
    if (error instanceof Prisma.PrismaClientKnownRequestError && error.code === 'P2021') {
      console.error('❌ GET /api/salles: The `Classroom` table does not exist. Please run `prisma migrate dev`.');
...
export async function POST(request: Request) {
  console.log('➡️ POST /api/salles: Received request');
  try {
    const body = await request.json();
    console.log('📝 POST /api/salles: Request body:', body);
    const validatedData = createClassroomSchema.parse(body);
    console.log('✅ POST /api/salles: Validation successful:', validatedData);

    const newClassroom = await prisma.classroom.create({
      data: validatedData,
    });
    console.log('⬅️ POST /api/salles: Success, created classroom:', newClassroom);
    return NextResponse.json(newClassroom, { status: 201 });
  } catch (error: any) {
    if (error instanceof z.ZodError) {
      console.error('❌ POST /api/salles: Validation error:', error.errors);
      return NextResponse.json({ message: 'Données invalides', errors: error.errors }, { status: 400 });
    }
    if (error instanceof Prisma.PrismaClientUnknownRequestError) {
        console.error("❌ POST /api/salles: An unknown Prisma error occurred. This often indicates a schema mismatch. Please run `prisma migrate dev`.", error);
    ...
    console.error('❌ POST /api/salles: General error creating classroom:', error);
    if (error.stack) {
        console.error("Stack trace:", error.stack);
    }
